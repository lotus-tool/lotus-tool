...
public class LifeLTSBuilder {
    ...
    public static List<Component> builderLTS(HmscComponent hmscComponent,
        ...
        List<BmscBlock> objectList = getAllBlockDSWithOutRepetition(hmscComponent);

       // Map<String, Component> createdComponentsWithIndividualLTSMap = new HashMap<>(createdComponentsWithIndividualLTS.size());
      //  creabuildComponentsWithIndividualLTSInMaptedComponentsWithIndividualLTSMap = parseToMap(createdComponentsWithIndividualLTS);

        Component currentIndividualComponent = new Component();

        Map<String, List<String>> trasitionsHmscInMapBySrcHmsc;

        trasitionsHmscInMapBySrcHmsc = buildTrasitionsInMap(hmscComponent.getTransitionMSCList());




        List<Component> componentListWithOutTals = concatIndividualLTSsWithOutTalTrasition( individualLTSMapByObject, objectList);

         addTals(componentListWithOutTals, hmscComponent.getTransitionMSCList());
    ...
    private static void addTals(List<Component> componentListWithOutTals, List<TransitionMSC> transitionsHmsc) {
        ...
        for(TransitionMSC transitionMSC : transitionsHmsc){
            ...
            GenericElement srcGenericElement = null;
            GenericElement dstGenericElement = null;

            if(transitionMSC.getSource() instanceof GenericElementView){

                srcGenericElement =  ((GenericElementView) transitionMSC.getSource()).getGenericElement();

            }else  {
                srcGenericElement = (GenericElement) transitionMSC.getSource();
            }

            if(transitionMSC.getDestiny() instanceof GenericElementView){

                dstGenericElement =  ((GenericElementView) transitionMSC.getDestiny()).getGenericElement();

            }else  {
                dstGenericElement = (GenericElement) transitionMSC.getDestiny();
            }



            String srcHmscLabel = srcGenericElement.getLabel();
            String dstHmscLabel = dstGenericElement.getLabel();
    ...
    private static void updateValueMap(State src, GenericElement genericElement) {
        if(genericElement instanceof HmscBlock){
                HmscBlock hmscBlock = (HmscBlock) genericElement;
                src.setValue("isExceptional",hmscBlock.isExceptional());
            }else {
                src.setValue("isInterceptionNode",true);
    ...
    private static List<Component> concatIndividualLTSsWithOutTalTrasition(
                                                                Map<String, List<Component>> individualLTSMapByObject,
                                                                List<BmscBlock> objectList
                                                                ) {
        ...
        for(BmscBlock object : objectList){
...
//        for(Map.Entry<String, List<Component>> entry : individualLTSMapByObject.entrySet()){
//            if(entry.getKey().contains("InterceptionNode")){
//                currentComponentByObjectOutTalTrasition = new Component();
//                List<Component> componentListByObject = entry.getValue();
//
//                currentComponentByObjectOutTalTrasition.setName(entry.getKey().concat("_life"));
//
//                for(Component componentByObject : componentListByObject){
//
//                    addStatesAndTrasitionIn(componentByObject, currentComponentByObjectOutTalTrasition);
...
//        }
    ...
    private static void addStatesAndTrasitionIn(Component componentSrc, Component componentDst) {
       ...
       if(componentSrc.getTransitionsList().size() == 0){
           ...
           State s =componentDst.newState(newIdSrc);

           if(oldSrcState.getValue("isExceptional")!= null){
               s.setValue("isExceptional",oldSrcState.getValue("isExceptional"));
           }

           if(oldSrcState.getValue("isInterceptionNode")!= null){
               s.setValue("isInterceptionNode",oldSrcState.getValue("isInterceptionNode"));
           }
       ...
       for(Transition trasitionSrc : componentSrc.getTransitionsList()){
           ...
           if(componentDst.getStateByID(newIdSrc)!=null){
               ...
               if(oldSrcState.getValue("isExceptional")!= null){
                   newSrcState.setValue("isExceptional",oldSrcState.getValue("isExceptional"));
               }

               if(oldSrcState.getValue("isInterceptionNode")!= null){
                   newSrcState.setValue("isInterceptionNode",oldSrcState.getValue("isInterceptionNode"));
               }
           ...
           if(componentDst.getStateByID(newIdDst)!=null){
               ...
               if(oldDstState.getValue("isExceptional")!= null){
                   newDstState.setValue("isExceptional",oldDstState.getValue("isExceptional"));
               }

               if(oldDstState.getValue("isInterceptionNode")!= null){
                   newDstState.setValue("isInterceptionNode",oldDstState.getValue("isInterceptionNode"));
               }
           }
...
//                                             Queue<HmscBlock> hmscQueue,
//                                             HmscComponent selectedComponentBuildDS,
//                                             Map<String, List<String>> mapIndividualLTS) {
//
//        HmscBlock initialHmsc = selectedComponentBuildDS.getInitialHmscBlock();
//
...
//    }
    ...
    private static Component concat(Map<String, List<String>> trasitionsInMap,
                                    Queue<HmscBlock> hmscHmscBlockQueue, HmscComponent selectedComponentBuildDS,
                                    Map<String, List<String>> mapIndividualLTS) {

        Component component = new Component();

        HmscBlock firstHmscHmscBlock = hmscHmscBlockQueue.peek();

        List<String>  labelAndDstHmscStringsList = trasitionsInMap.get(firstHmscHmscBlock.getLabel());
        ...
        for(String labelAndDstHmscString : labelAndDstHmscStringsList){
            ...
            component = concat(firstHmscHmscBlock.getLabel(), labelTrasition, dstHmscLabel);
        }

        hmscHmscBlockQueue.remove();
    ...
    private static Map<String, List<String>> buildTrasitionsInMap(List<TransitionMSC> transitions) {
        ...
        for(TransitionMSC transitionMSC : transitions){
            ...
            String labelAndDstHmsc = transitionMSC.getLabel().concat("$$").concat(dstGenericElement.getLabel());

            if(stringListMap.containsKey(srcGenericElement.getLabel())){
                stringListMap.get(srcGenericElement.getLabel()).add(labelAndDstHmsc);
            }else {
                List<String> labelAndDstHmscStrings = new ArrayList<>();
                labelAndDstHmscStrings.add(labelAndDstHmsc);
                stringListMap.put(srcGenericElement.getLabel(),labelAndDstHmscStrings);
            }
    ...
    private static List<BmscBlock> getAllBlockDSWithOutRepetition(HmscComponent standardModeling) {
        Map<String, BmscBlock> blocksWithOutRepetition = new HashMap<>();

        for(HmscBlock hmscHmscBlock : standardModeling.getHmscBlockList()){
            BmscComponent currentBmscComponent = hmscHmscBlock.getBmscComponet();
                for(BmscBlock bmscBlock : currentBmscComponent.getBmscBlockList()){
                    blocksWithOutRepetition.put(bmscBlock.getLabel(), bmscBlock);
                }

        }

        return new ArrayList<BmscBlock>(blocksWithOutRepetition.values());
    }
