...
import br.uece.lotus.msc.api.viewer.bMSC.ComponentDSView;
import br.uece.lotus.msc.api.viewer.bMSC.ComponentDSViewImpl;
import br.uece.lotus.msc.api.viewer.transition.TransitionMSCView;
import br.uece.lotus.msc.api.window.WindowMSC;
import br.uece.lotus.msc.designer.blockDiagramModeling.strategy.*;
...
public class DesingWindowImplBlockMSC extends AnchorPane implements WindowMSC {
    //Principais da Tela
    ...
    public BmscComponent getBmscComponent() {
        return mViewer.getBmscComponent();
    }
    ...
    public void setBmscComponent(BmscComponent bmscComponent) {
        mViewer.setComponentDS(bmscComponent);
    }
    ...
    public String getTitle() {
        return mViewer.getBmscComponent().getName();
    }
    ...
    public DesingWindowImplBlockMSC(){
  ...
  // INICIANDO COMPONENTES DA INTERFACE
   ////////////////////////////////////////////////////////////////////
        ...
        mSaveAsPNG.setOnAction((ActionEvent event) -> {
            ...
            fileChooser.setInitialFileName(mViewer.getBmscComponent().getName() + ".png");
    ...
    public void updateContID() {
        int aux = -1;
        for (BmscBlock b : mViewer.getBmscComponent().getBmscBlockList()) {
            if (b.getID() > aux) {
                aux = b.getID();
            }
        }
    ...
    private void applySelectedStyles(Object mComponentSelecionado) {
        if(mComponentSelecionado instanceof BlockDSView){
            BmscBlock b = ((BlockDSView) mComponentSelecionado).getBlockDS();
            b.setBorderWidth(3);
            b.setBorderColor("blue");
            b.setTextColor("blue");
            b.setTextStyle(BmscBlock.mTextStyleBold);
        }
    ...
    private void removeSelectedStyles(Object mComponentSelecionado) {
        if(mComponentSelecionado instanceof BlockDSView){
            ...
            b.setTextStyle(BmscBlock.mTextStyleNormal);
        }
        else if(mComponentSelecionado instanceof TransitionMSCView){
            TransitionMSC t = ((TransitionMSCView)mComponentSelecionado).getTransition();
            t.setBorderWidth(1);
    ...
    public void updateSequenceTransition(/*TransitionMSCView t*/){
        List<TransitionMSC> allTrasition = mViewer.getBmscComponent().getAllTransitions();
